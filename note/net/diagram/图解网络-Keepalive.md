...
- [一、HTTP的Keep-Alive](#%E4%B8%80http%E7%9A%84keep-alive)
- [二、TCP的Keepalive](#%E4%BA%8Ctcp%E7%9A%84keepalive)
...
![png](images/kl-请求-应答.png)

由于 HTTP 是基于 TCP 传输协议实现的，客户端与服务端要进行 HTTP 通信前，需要先建立 TCP 连接，然后客户端发送 HTTP 请求，服务端收到后就返回响应，至此「请求-应答」的模式就完成了，随后就会释放 TCP 连接。

![png](images/kl-一个HTTP请求.png)

如果每次请求都要经历这样的过程：建立 TCP -> 请求资源 -> 响应资源 -> 释放连接，那么此方式就是 **HTTP 短连接**，如下图：

![png](images/kl-HTTP短连接.png)
...
![png](images/kl-右边为HTTP流水线机制.png)
...
![png](images/kl-HTTP长连接超时.png)

## 二、TCP的Keepalive

TCP 的 Keepalive 这东西其实就是 **TCP 的保活机制**

![png](images/kl-TCP的Keepalive.png)
