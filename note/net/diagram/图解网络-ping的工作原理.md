...
- [一、IP协议的助手 —— ICMP 协议](#%E4%B8%80ip%E5%8D%8F%E8%AE%AE%E7%9A%84%E5%8A%A9%E6%89%8B--icmp-%E5%8D%8F%E8%AE%AE)
- [二、查询报文类型](#%E4%BA%8C%E6%9F%A5%E8%AF%A2%E6%8A%A5%E6%96%87%E7%B1%BB%E5%9E%8B)
- [三、差错报文类型](#%E4%B8%89%E5%B7%AE%E9%94%99%E6%8A%A5%E6%96%87%E7%B1%BB%E5%9E%8B)
- [四、ping —— 查询报文类型的使用](#%E5%9B%9Bping--%E6%9F%A5%E8%AF%A2%E6%8A%A5%E6%96%87%E7%B1%BB%E5%9E%8B%E7%9A%84%E4%BD%BF%E7%94%A8)
- [五、traceroute —— 差错报文类型的使用](#%E4%BA%94traceroute--%E5%B7%AE%E9%94%99%E6%8A%A5%E6%96%87%E7%B1%BB%E5%9E%8B%E7%9A%84%E4%BD%BF%E7%94%A8)
...
为了给大家说清楚上面的目标不可达的原因，**牺牲自己给大家送 5 次外卖。**
...
第一次送外卖时，小区里只有 A 和 B 区两栋楼，但送餐地址写的是 C 区楼，表示头上很多问号，压根就没这个地方。
...
第五次送外卖时，这次是个吃播博主了 100 份外卖，但是吃播博主要求一次性要把全部外卖送达，一台电动车装不下呀，这样就没办法送达了。

*正常版本：*

发送端主机发送 IP 数据报时，将 IP 首部的**分片禁止标志位**设置为`1`。根据这个标志位，途中的路由器遇到超过 MTU 大小的数据包时，不会进行分片，而是直接抛弃。随后，通过一个 ICMP 的不可达消息类型，**代码为 4**
的报文，告知发送端主机。
...
好比，本可以过条马路就能到的地方，但不知道，所以绕了一圈才到，后面知道后，下次就不会那么**傻**再绕一圈了。
...
![png](images/主机A的ICMP回送请求报文示意图.png)

然后，由 ICMP 协议将这个数据包连同地址 192.168.1.2 一起交给 IP 层。IP 层将以 192.168.1.2 作为**目的地址**，本机 IP 地址作为**源地址**，**协议**字段设置为 `1` 表示是 `ICMP`
协议，在加上一些其他控制信息，构建一个 `IP` 数据包。

![png](images/主机A的IP层数据包示意图.png)

接下来，需要加入 `MAC` 头。如果在本地 ARP 映射表中查找出 IP 地址 192.168.1.2 所对应的 MAC 地址，则可以直接使用；如果没有，则需要发送 `ARP` 协议查询 MAC 地址，获得 MAC
地址后，由数据链路层构建一个数据帧，目的地址是 IP 层传过来的 MAC 地址，源地址则是本机的 MAC 地址；还要附加上一些控制信息，依据以太网的介质访问规则，将它们传送出去。

![png](images/主机A的MAC层数据包示意图.png)
...
![png](images/主机B的ICMP回送响应报文示意图.png)
...
![png](images/主机Aping主机B期间发送的事情示意图.png)
...
![png](images/MTU路径发现（UDP的情况下）示意图.png)
