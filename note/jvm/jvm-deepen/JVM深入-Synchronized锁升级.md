...
- [一、无锁](#%E4%B8%80%E6%97%A0%E9%94%81)
- [二、偏向锁](#%E4%BA%8C%E5%81%8F%E5%90%91%E9%94%81)
  - [2.1 偏向锁原理](#21-%E5%81%8F%E5%90%91%E9%94%81%E5%8E%9F%E7%90%86)
  - [2.2 偏向锁升级](#22-%E5%81%8F%E5%90%91%E9%94%81%E5%8D%87%E7%BA%A7)
  - [2.3 批量重偏向](#23-%E6%89%B9%E9%87%8F%E9%87%8D%E5%81%8F%E5%90%91)
  - [2.4 批量撤销](#24-%E6%89%B9%E9%87%8F%E6%92%A4%E9%94%80)
  - [2.5 总结](#25-%E6%80%BB%E7%BB%93)
- [三、轻量级锁](#%E4%B8%89%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81)
  - [3.1 轻量级锁原理](#31-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%86)
  - [3.2 轻量级锁重入](#32-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E9%87%8D%E5%85%A5)
  - [3.3 轻量级锁升级**](#33-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E5%8D%87%E7%BA%A7)
  - [3.4 总结**](#34-%E6%80%BB%E7%BB%93)
- [四、重量级锁**](#%E5%9B%9B%E9%87%8D%E9%87%8F%E7%BA%A7%E9%94%81)
  - [4.1 Monitor](#41-monitor)
  - [4.2 重量级锁原理](#42-%E9%87%8D%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%86)
...
![png](images/批量重偏向结果图2.png)

线程`t2`结束后，第1-19的对象释放轻量级锁变为无锁不可偏向状态，第20-30的对象状态为偏向锁、但从偏向`t1`改为偏向`t2`，第31-40的对象因为没有被线程`t2`访问所以保持偏向线程`t1`不变。

有人可能发现，这回图的颜色不太对，是因为我使用上述代码未能得到相同的结果，运行结果如下：

![png](images/批量重偏向结果图1-失败.png)

![png](images/批量重偏向结果图2-失败.png)
...
3. 轻量级锁的释放同样使用了CAS操作，尝试将`displaced mark word` 替换回`mark word`，这时需要检查锁对象的`mark word`中`lock record`指针是否指向当前线程的锁记录：
...
上面在最后一次打印前将主线程休眠5秒的原因是锁的释放过程需要一定的时间，如果在线程执行完成后直接打印对象内存布局，对象可能仍处于重量级锁状态。
...
- `EntryList`：候选者列表，当`owner`解锁时会将`cxq`队列中的线程移动到该队列中（这个设计或许是因为候选者有可能是来着`cxq`或者`WaitSet`？谁知道呢）
- `OnDeck`：在将线程从`cxq`移动到`EntryList`时，会指定某个线程为Ready状态（即`OnDeck`），表明它可以竞争锁，如果竞争成功那么成为`owner`线程，如果失败则放回`EntryList`中
